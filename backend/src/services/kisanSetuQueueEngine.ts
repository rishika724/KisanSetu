 ...
 */
export const UNLOADING_DURATIONS_MINUTES = {
  BULLOCK_CART: 25,
  TRACTOR: 15,
  TRUCK: 10
} as const;

export type VehicleTypeKey = keyof typeof UNLOADING_DURATIONS_MINUTES;

export type CongestionLevel = 'NORMAL' | 'MODERATE' | 'CONGESTED';

export type QueueStageStatus = 'BOOKED' | 'STAGING' | 'MANDI_GATE' | 'INSPECTION' | 'COMPLETED';
...
export async function calculateAvailableCapacity(
  ...
  let bookings: Array<{ vehicleType: string }> = [];
  ...
  if (isDbActive()) {
    ...
    const slot = memoryStore.slots.find((s) => s.centerId === centerId && s.date === date && s.timeWindow === timeWindow);
    if (slot) {
  ...
  for (const b of bookings) {
    const type = b.vehicleType as VehicleTypeKey;
    if (UNLOADING_DURATIONS_MINUTES[type]) {
      usedMinutes += UNLOADING_DURATIONS_MINUTES[type];
  ...
  let congestionLevel: CongestionLevel = 'NORMAL';
  ...
  const canAcceptVehicle = {
    BULLOCK_CART: remainingMinutes >= UNLOADING_DURATIONS_MINUTES.BULLOCK_CART,
    TRACTOR: remainingMinutes >= UNLOADING_DURATIONS_MINUTES.TRACTOR,
    TRUCK: remainingMinutes >= UNLOADING_DURATIONS_MINUTES.TRUCK
  };
 ...
 */
export function updateFarmerQueueState(
  farmerLat: number,
  farmerLng: number,
  centerLat: number,
  centerLng: number
 ...
 */
export function generateKisanSetuOfflineToken(
  farmerId: string,
  slotId: string,
  ...
  const tokenHash = crypto.createHash('sha256').update(rawMessage).digest('hex').toUpperCase();
